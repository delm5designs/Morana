...
- `SKILL.md` - agent-invocable skill wrapper, so this system works as a downloadable Claude Code skill.
...
**`ui_kits/website/`** - the marketing site as a click-through kit: `index.html` plus `Home`, `Programs`, `Approach`, `About`, `Contact`, shared `parts.jsx`, and `copy.js` holding every EN and BG string. See `ui_kits/website/README.md`.

**`slides/`** - the slide template: seven 16:9 types (`TitleSlide`, `SectionSlide`, `StatementSlide`, `ComparisonSlide`, `ProcessSlide`, `ProgramSlide`, `ClosingSlide`) plus a 1:1 `QuoteSquare`, one preview HTML each. Ink covers and dividers, cream working slides. See `slides/README.md`.
